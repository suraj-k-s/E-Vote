...
if($_SERVER['REQUEST_METHOD']=="GET")
{
    ...
    $qry14 = "SELECT * FROM tbl_campaigning c INNER JOIN tbl_candidate cd on c.candidate_id=cd.candidate_id INNER JOIN tbl_user u on cd.user_id=u.user_id WHERE cd.ward_id in(SELECT w.ward_id FROM tbl_ward w INNER JOIN tbl_candidate c on w.ward_id=c.ward_id INNER JOIN tbl_user us on us.ward_id=c.ward_id WHERE us.user_id='".$id."') ORDER BY c.	campaigning_id DESC";
